...
import com.chingchan.theClap.databinding.RvCompWriteCatBinding
import com.chingchan.theClap.ui.compliment.data.CompCatData

class CompWriteCatRecyclerAdapter() : RecyclerView.Adapter<CompWriteCatRecyclerAdapter.RecyclerViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerViewHolder {
        ...
        return RecyclerViewHolder(RvCompWriteCatBinding.inflate(LayoutInflater.from(parent.context),parent,false))
    }
    ...
    inner class RecyclerViewHolder(private val binding: RvCompWriteCatBinding) : RecyclerView.ViewHolder(binding.root){
        //뷰홀더: 내가 넣고자하는 data를 실제 레이아웃의 데이터로 연결시키는 기능
        fun bind(compCatData: CompCatData){//view와 데이터를 연결시키는 함수-/>뷰에 데이터 넣음

            with(binding){
                btnCat.isSelected = compCatData.isSelect
                btnCat.text = compCatData.name
            }
        }
        ...
        init {
            with(binding){
                btnCat.setOnClickListener {
                    clickListener?.onClick(adapterPosition, "CATEGORY")
                }
            }
        }
    }
