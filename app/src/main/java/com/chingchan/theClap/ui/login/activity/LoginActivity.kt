...
//                    UserData.setUserName(applicationContext, loginUserData.name)
    ...
    private fun guestLogin() {
        val apiObjectCall = ApiObject.getLoginService.getGuestLogin()

        apiObjectCall.enqueue(object: Callback<GuestLoginUserResData> {
            override fun onResponse(call: Call<GuestLoginUserResData>, response: Response<GuestLoginUserResData>) {
                // 추후 성공 시 여기 코드로 리팩토링
                if(response.isSuccessful) {
                    val loginUserData = response.body()?.data

                    val guestUserData = MembershipUserData(loginType = "GUEST", socialId = loginUserData!!.guest)   // 게스트 로그인 데이터

                    loginCheck(guestUserData)   // 로그인
                }else{
                    customToast.showCustomToast("게스트 로그인에 실패했습니다.", this@LoginActivity)
                }
            }

            override fun onFailure(call: Call<GuestLoginUserResData>, t: Throwable) {
                customToast.showCustomToast("Call Failed", this@LoginActivity)
            }
        })
    }
