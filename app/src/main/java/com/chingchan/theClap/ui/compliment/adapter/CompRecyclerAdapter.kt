...
class CompRecyclerAdapter() : RecyclerView.Adapter<CompRecyclerAdapter.RecyclerViewHolder>() {
    ...
    private lateinit var compImageRecyclerAdapter: CompImageRecyclerAdapter   // 게시글 이미지 리사이클러뷰 어뎁터
    ...
    inner class RecyclerViewHolder(private val binding: RvCompBinding) : RecyclerView.ViewHolder(binding.root){
        ...
        fun bind(compData: CompData){//view와 데이터를 연결시키는 함수-/>뷰에 데이터 넣음
            ...
            with(binding){
                ...
                catType.text = compData.typeName    // 타입 명
                ...
                // 게시글 이미지 리사이클러뷰
                compImageRecyclerAdapter = CompImageRecyclerAdapter()
                rvImage.adapter = compImageRecyclerAdapter
                ...
                compImageRecyclerAdapter.differ.submitList(compImageDataList.toMutableList())
            }
        }
