    ...
    inner class RecyclerViewHolder(private val binding: RvFragCompCatBinding) : RecyclerView.ViewHolder(binding.root){
        ...
        fun bind(compCatData: CompCatData){//view와 데이터를 연결시키는 함수-/>뷰에 데이터 넣음
            ...
            with(binding){
                ...
                when(compCatData.name){
                    "전체" -> catImage.setImageResource(R.drawable.ic_category_all)
                    "인성/성품" -> catImage.setImageResource(R.drawable.ic_category_01)
                    "재능/능력" -> catImage.setImageResource(R.drawable.ic_category_02)
                    "선행" -> catImage.setImageResource(R.drawable.ic_category_03)
                    "목표달성" -> catImage.setImageResource(R.drawable.ic_category_04)
                    "격려/기타" -> catImage.setImageResource(R.drawable.ic_category_05)
                    else -> catImage.setImageResource(R.drawable.ic_close)
                }

                if(compCatData.new){
                    catDot.visibility = View.VISIBLE
                }else{
                    catDot.visibility = View.GONE
                }
            }
        }
