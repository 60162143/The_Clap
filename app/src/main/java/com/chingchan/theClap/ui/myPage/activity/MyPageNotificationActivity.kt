...
import com.chingchan.theClap.databinding.ActiMypageNotificationBinding
...
import com.chingchan.theClap.ui.myPage.adapter.MyPageNotificationRecyclerAdapter
...
class MyPageNotificationActivity : AppCompatActivity() {
    private lateinit var binding: ActiMypageNotificationBinding

    private var notificationRecyclerAdapter: MyPageNotificationRecyclerAdapter = MyPageNotificationRecyclerAdapter()    // 내소식 리사이클러뷰 어뎁터

    private var otherActiveList: ArrayList<NotificationData> = ArrayList()  // '알림' 전체 데이터
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActiMypageNotificationBinding.inflate(layoutInflater)
        ...
        with(binding){
            ...
            // 내소식 클릭 리스너
            notificationRecyclerAdapter.clickListener = object : MyPageNotificationRecyclerAdapter.OnClickListener{
                    ...
                    if(type == "TOTAL"){  // 전체 영역 클릭 시
                        ...
                        if(curActiveType == "OTHER_ACTIVE"){    // Type이 '알림'인 경우
                            ...
                            if(otherActiveList[position].logTypeId == NotificationType.TYPE02.code){    // 댓글 관련
                                ...
                                setResult(1001, intent) // 인기글 관련 알림 클릭으로 왔을 경우 Result Code
                                finish()
                            }
                        }else{  // Type이 '활동'인 경우
    ...
    private fun setToggleBtn(tf: Boolean){
        with(binding){
            if(tf){
                ...
                btnNoti.typeface = ResourcesCompat.getFont(applicationContext, R.font.pretendard_extrabold)
                btnAct.typeface = ResourcesCompat.getFont(applicationContext, R.font.pretendard_regular)
                ...
                btnNoti.typeface = ResourcesCompat.getFont(applicationContext, R.font.pretendard_regular)
                btnAct.typeface = ResourcesCompat.getFont(applicationContext, R.font.pretendard_extrabold)
    ...
    private fun getNotificationData(){
        val apiObjectCall = ApiObject.getNotificationService.getLog(authorization = "Bearer " + UserData.getAccessToken(applicationContext))
        ...
        apiObjectCall.enqueue(object: Callback<NotificationResData> {
            override fun onResponse(call: Call<NotificationResData>, response: Response<NotificationResData>) {
                ...
                if(response.isSuccessful) {
                    ...
                    if(status == "success"){
                        ...
                        customToast.showCustomToast("내소식 정보 조회를 못했습니다.", this@MyPageNotificationActivity)
                    }
                }else{
                    customToast.showCustomToast("내소식 정보 조회를 못했습니다.", this@MyPageNotificationActivity)
                }
            ...
            override fun onFailure(call: Call<NotificationResData>, t: Throwable) {
                customToast.showCustomToast("Call Failed", this@MyPageNotificationActivity)
            }
        })
    }
    ...
    private fun deleteNotificationData(logIds: ArrayList<Int>){
        val apiObjectCall = ApiObject.getNotificationService.deleteLog(authorization = "Bearer " + UserData.getAccessToken(applicationContext)
            , logIds = logIds)
        ...
        apiObjectCall.enqueue(object: Callback<NotificationDeleteResData> {
            override fun onResponse(call: Call<NotificationDeleteResData>, response: Response<NotificationDeleteResData>) {
                ...
                if(response.isSuccessful) {
                    ...
                    customToast.showCustomToast(response.body()?.data.toString(), this@MyPageNotificationActivity)
                }else{
                    customToast.showCustomToast("내소식 정보 삭제를 못했습니다.", this@MyPageNotificationActivity)
                }
            }
    ...
    private fun readNotificationData(logId: Int){
        val apiObjectCall = ApiObject.getNotificationService.readLog(authorization = "Bearer " + UserData.getAccessToken(applicationContext)
            , logId = logId)
            ...
            override fun onResponse(call: Call<NotificationReadResData>, response: Response<NotificationReadResData>) {
                ...
                if(response.isSuccessful) {
                    ...
                    if(status == "success"){
                        ...
                        customToast.showCustomToast("내소식 '읽음' 처리를 못했습니다.", this@MyPageNotificationActivity)
                    }
                }else{
                    customToast.showCustomToast("내소식 '읽음' 처리를 못했습니다.", this@MyPageNotificationActivity)
                }
        ...
        apiObjectCall.enqueue(object: Callback<CompCatResData> {
            override fun onResponse(call: Call<CompCatResData>, response: Response<CompCatResData>) {
                ...
                if(response.isSuccessful) {
                    ...
                    customToast.showCustomToast("카테고리를 불러오지 못했습니다.", this@MyPageNotificationActivity)
                }
            }
            ...
            override fun onResponse(call: Call<CompOneResData>, response: Response<CompOneResData>) {
                ...
                if(response.isSuccessful) {
                    ...
                    if(status == "success"){
                        ...
                        val intent = Intent(this@MyPageNotificationActivity, ComplimentDetailActivity::class.java)

                        intent.putParcelableArrayListExtra("compCatData", compCatList)
                        intent.putExtra("compData", compData)
                        startActivity(intent)
                    }else{
                        customToast.showCustomToast("게시물 조회를 못했습니다.", this@MyPageNotificationActivity)
                    }
    ...
    private fun getFollowMatchCount(){
        val apiObjectCall = ApiObject.getMyPageService.getFollowMatchCount(authorization = "Bearer " + UserData.getAccessToken(applicationContext))
            ...
            override fun onResponse(call: Call<FollowMatchCountResData>, response: Response<FollowMatchCountResData>) {
                ...
                if(response.isSuccessful) {
                    ...
                    if(status == "success"){
                        ...
                        customToast.showCustomToast("서로 팔로잉한 수 조회를 못했습니다.", this@MyPageNotificationActivity)
                    }
            ...
            override fun onResponse(call: Call<ProfileResData>, response: Response<ProfileResData>) {
                ...
                if(response.isSuccessful) {
                    ...
                    if(status == "success"){
                        ...
                        val intent = Intent(this@MyPageNotificationActivity, MyPageProfileOtherActivity::class.java)
                        intent.putExtra("profileData", profileData)

                        startActivity(intent)
                    }else{
                        customToast.showCustomToast("프로필 정보 조회를 못했습니다.", this@MyPageNotificationActivity)
                    }
                    ...
                    errorData?.code.let{
                        if(it == ErrorCode.S00011.name){    // 프로필 데이터 조회 x ( 차단 or 삭제...?)
                            customToast.showCustomToast(ErrorCode.S00011.message, this@MyPageNotificationActivity)
                        }else{
                            customToast.showCustomToast("프로필 정보 조회를 못했습니다.", this@MyPageNotificationActivity)
                        }
                    }
    ...
    private fun setData(){
        ...
        }else if(curActiveType == "USER_ACTIVE"){   // Type이 '활동'인 경우
            if(curIsRead){  // '읽음' 토글 활성화 경우
                setLayout(userActiveIsReadList.isEmpty())    // 레이아웃 SET
